...
<option value="courseListServletpx?addID=1">增序   </option>
<option value="courseListServletpx?addID=2">降序   </option>
</select></td>
...
<%for(int i=0;i<course.size();i++){%>  
			<tr>
				<th ><%=course.get(i).getId() %></th>
				<td ><%=course.get(i).getName() %></td>
				<td><%=course.get(i).getType() %> </td>
				<td><%=course.get(i).getInfo() %></td>
				<td><%=course.get(i).getPublisher() %></td>
                <td><%=course.get(i).getCdate() %></td>
                
                <td style="width: 45px;"><a href="SearchById?courseId=<%=course.get(i).getId() %>">修改</a></td>
                 <td style="width: 45px; "><a href="DelCourseServlet?Id=<%=course.get(i).getId()%>"  onClick="return confirm('真的要删除?')">删除</a></td>
			</tr>
			<%}%>
